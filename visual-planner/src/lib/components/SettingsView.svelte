...
<div class="contentbox settings-box">
    ...
    <div class="subsection">
      <div class="permission-row">
        ...
        {:else}
          ...
          <span>visual-planner does not have permission to view your calendars and cannot display any of your calendar events.</span>
        {/if}
      </div>
      <div class="permission-row">
        {#if permissions.drive_appdata}
          <span class="material-icons perm-icon granted">verified_user</span>
          <span>visual-planner has permission to manage its own application data in your Google Drive.</span>
        {:else}
          <span class="material-icons perm-icon denied">warning</span>
          <span>visual-planner does not have permission to manage its own application data in your Google Drive and cannot save your settings.</span>
        {/if}
      </div>
      <p><a href="https://myaccount.google.com/permissions" target="_blank" rel="noreferrer">Manage Permissions</a></p>
      <p>
        <a href="https://drive.google.com/drive/my-drive" target="_blank" rel="noreferrer">Manage Application Data</a>
  ...
  <section>
    ...
    <div class="subsection">
      ...
      <div class="subsection">
        <label class="field-row">
          <input type="number" min="-12" max="12" bind:value={appdata.auto_scroll_offset} oninput={markDirty} class="num-input" />
          Auto-scroll offset.
        </label>
        <label class="field-row">
          <input type="number" min="1" max="12" bind:value={appdata.first_month} oninput={markDirty} class="num-input" />
          First month of year.
        </label>
      </div>
      <label class="field-row checkbox-row">
      ...
      <label class="field-row">
        <input type="number" min="0" max="6" bind:value={appdata.first_day_of_week} oninput={markDirty} class="num-input" />
        First day of week. (0–6)
      </label>
      <label class="field-row">
        <input type="number" min="50" max="200" bind:value={appdata.font_scale_pc} oninput={markDirty} class="num-input" />
        % Font scale.
      </label>
      <label class="field-row">
        <input type="number" min="0" max="1" step="0.1" bind:value={appdata.past_opacity} oninput={markDirty} class="num-input" />
  ...
  <section>
    ...
    <div class="subsection">
      ...
      <div class="subsection">
        <label class="field-row checkbox-row">
          <input type="checkbox" bind:checked={appdata.single_day_as_multi_day} onchange={markDirty} />
          Single day as multi-day (vertical bar).
        </label>
      </div>
      <label class="field-row checkbox-row">
        <input type="checkbox" bind:checked={appdata.text_on_singleday_events} onchange={markDirty} />
        Show text on single-day events.
      </label>
      <label class="field-row checkbox-row">
        <input type="checkbox" bind:checked={appdata.text_on_multiday_events} onchange={markDirty} />
        Show text on multi-day events.
      </label>
      <label class="field-row">
        <input type="number" bind:value={appdata.scale_of_multiday_events} oninput={markDirty} class="num-input" />
  ...
  <section class="links-section">
    <p><a href="https://visual-planner.github.io" target="_blank" rel="noreferrer">visual-planner home</a></p>
    <p><a href="https://groups.google.com/group/visual-planner-discuss" target="_blank" rel="noreferrer">visual-planner discussion group</a></p>
  </section>
</div>
