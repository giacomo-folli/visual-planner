...
export function setGridView(
  ...
  if (sel.column) { next.column = { checked: true }; delete next.list; }
  if (sel.list) { next.list = { checked: true }; delete next.column; }
  if (sel.expand) { next.expand = { checked: true }; delete next.collapse; }
  if (sel.collapse) { next.collapse = { checked: true }; delete next.expand; }
  if (sel.darktog) {
...
export class VpConfiguration {
  ...
  constructor(
    ...
    appdata: AppData,
    clientId = "186424320143-vb1h85auvvpnojvmeg9gi6lv9aan4ggi.apps.googleusercontent.com"
  ) {
  ...
  private loadPermissionsThen(doThis: () => void): void {
    // @ts-expect-error – google global injected via CDN
    google.accounts.oauth2
      .initTokenClient({
        ...
        callback: () => {
          ...
          // @ts-expect-error - beacuse yes
          if (google.accounts.oauth2.hasGrantedAllScopes(token, "https://www.googleapis.com/auth/calendar.readonly"))
            this.permissions.view_calendars = true;
          // @ts-expect-error - beacuse yes
          if (google.accounts.oauth2.hasGrantedAllScopes(token, "https://www.googleapis.com/auth/drive.appdata"))
            this.permissions.drive_appdata = true;
...
export class VpGCal {
  ...
  addPublicCalendar(id: string, name: string, colour: string, textColour: string): void {
    this.calendarlist.push(
      new VpCalendar({ id, summary: name, backgroundColor: colour, foregroundColor: textColour, selected: true }, this)
    );
  }
  ...
  private loadGCalSettings(): void {
    ...
    gapi.client
      ...
      .then((response: { result: { kind: string; id: string; value: string } }) => {
        if (response.result?.kind === "calendar#setting" && response.result.id === "format24HourTime")
    ...
    gapi.client
      ...
      .then((response: { result: { kind: string; event: typeof this.eventColours } }) => {
        if (response.result?.kind === "calendar#colors") this.eventColours = response.result.event;
      });
  }
  ...
  _getIsoSpan() { return this.isoSpan; }
  _getCfg() { return this.cfg; }
  _getEventColours() { return this.eventColours; }
  _getFAdd() { return this.fAdd; }
  _getFRemove() { return this.fRemove; }
  _getReqFailThen() { return this.reqFailThen; }
  _setReqFailThen(v: number) { this.reqFailThen = v; }
  _reAuth(cb: () => void, vpConfig: VpConfiguration) { vpConfig.authoriseThen(cb); }
...
export class VpCalendar implements VpCalendarData {
  ...
  private reqEvents(reqparams: Record<string, string | boolean>): void {
    ...
    gapi.client
      ...
      .then((response: { result: { items: GCalEventItem[]; nextPageToken?: string; nextSyncToken?: string } }) => {
  ...
  private makeEvent(item: GCalEventItem, reqparams: Record<string, string | boolean>): void {
    ...
    if (item.status === "cancelled") { this.gcal._getFRemove()(item.id); return; }
...
export class VpDiary {
  ...
  getPage(): VpMonth[] { return this.vpmonths; }
  getMonth(i: number): VpMonth { return this.vpmonths[i]; }

  sync(): void { this.gcal.syncAllCalEvents(); }
...
export class VpMonth {
  ...
  addEvent(day: VpDay, evt: VpEventData, border: LabelBorder): void {
    ...
    for (const lab of this.labels) {
      if (lab.evt === evt) { lab.setCellEnd(day.index, border); return; }
    }
...
export class VpLabel {
  ...
  updateLayout(slots: number[], view: GridViewState, month: VpMonth): void {
    ...
    if (this.multiboxstyle && this._month) {
      ...
      this.multiboxstyle[gridCross] = `${this._month.dayoffset + this._day + 2} / span ${this._span}`;
    }
  }
}
...
function removeEventFromOwner(
  owner: { labels?: VpLabel[] },
  id?: string
): void {
...
export class VpGrid {
  ...
  onclickExpand(view: GridViewState): GridViewState {
    return view.expand ? setGridView(view, { collapse: true }) : setGridView(view, { expand: true });
  }
